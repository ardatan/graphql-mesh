...
- Upstream gRPC `ServiceError`s are returned as GraphQL errors with `extensions.code: DOWNSTREAM_SERVICE_ERROR` and details under `extensions.grpc` (`code`, `status`, `details`, `metadata`)
