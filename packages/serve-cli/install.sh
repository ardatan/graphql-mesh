...
download_binary() {
  ...
  while [ -n "$RELEASES_URL" ]; do
    ...
    for RELEASE in $(echo "$RESPONSE" | jq -r '.[] | @base64'); do
      ...
      if echo "$BODY" | grep -q "@graphql-mesh/serve-cli@$PACKAGE_VERSION"; then
...
need_cmd() {
  if ! check_cmd "$1"; then
    err "Installation halted. Reason: [command not found '$1' - please install this command]"
  fi
}
