...
import type { TransportEntry, UnifiedGraphManagerOptions } from '@graphql-mesh/fusion-runtime';
import {
...
import { isUrl, mapMaybePromise } from '@graphql-mesh/utils';
...
import { type GatewayConfigContext2 } from './createGatewayCore.js';
...
export function createGatewayPlugins<TContext extends Record<string, any> = Record<string, any>>(
  config: GatewayConfig<TContext>,
  base: GatewayConfigContext2,
): {
  plugins: GatewayPlugin[];
  getSchema: () => MaybePromise<GraphQLSchema>;
  schemaInvalidator: () => void;
  contextBuilder: (context: any) => MaybePromise<any>;
  subgraphInformationHTMLRenderer: () => MaybePromise<string>;
} {
  const configContext = base;
  const onSubgraphExecuteHooks = base.onSubgraphExecuteHooks;
  const onDelegateHooks = base.onDelegateHooks;
  const disposableStack = base.disposableStack;
  ...
  return {
    plugins: [...basePlugins, ...(config.plugins?.(base) || []), ...extraPlugins],
    schemaInvalidator,
    getSchema,
    contextBuilder,
    subgraphInformationHTMLRenderer,
