...
): Promise<RAMLLoaderBundle> {
  const { operations, baseUrl, cwd, fetch } = await getJSONSchemaOptionsFromRAMLOptions(
    ramlLoaderOptions,
  );
  return createJSONSchemaLoaderBundle(name, {
    ...ramlLoaderOptions,
    operationHeaders:
      typeof ramlLoaderOptions.operationHeaders === 'object'
        ? ramlLoaderOptions.operationHeaders
        : {},
    baseUrl,
    operations,
    cwd,
