...
import BareRename from '../src/bareRename.js';
import WrapRename from '../src/wrapRename.js';
import RenameTransform from './../src/index.js';

const MODES: ('wrap' | 'bare')[] = ['wrap', 'bare'];
...
for (const MODE of MODES) {
  describe(`rename (${MODE})`, () => {
    ...
    const applyTransformation = (schema: GraphQLSchema, transform: BareRename | WrapRename) => {
      switch (MODE) {
        case 'wrap':
          return wrapSchema({
            schema,
        ...
        case 'bare':
          return transform.transformSchema(schema, {} as any);
      }
    };
