...
type PathConfigItem = ConstructorParameters<typeof HoistField>[1][0];

export default class HoistFieldTransform implements MeshTransform {
  public noWrap: boolean;
  private transforms: Transform[];

  constructor({ config }: MeshTransformOptions<YamlConfig.Transform['hoist']>) {
    this.noWrap = false;
  ...
  private getPathConfigItem(
    pathConfigItemFromConfig: YamlConfig.HoistFieldTransformObject['pathConfig'][0],
    filterArgsInPath: boolean
  ): PathConfigItem {
    if (typeof pathConfigItemFromConfig === 'string') {
      const pathConfigItem: PathConfigItem = {
