...
import type { HoistFieldTransformFieldPathConfig } from './types';
...
export default class WrapHoistField implements MeshTransform {
  ...
  constructor({ config }: MeshTransformOptions<YamlConfig.HoistFieldTransformConfig>) {
    this.transforms = config.fields.map(({ typeName, pathConfig, newFieldName, alias, filterArgsInPath = false }) => {
