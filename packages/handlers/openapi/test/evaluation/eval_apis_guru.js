...
async function readOas(limit) {
  const OASList = [];
  const paths = Glob.sync('tmp/**/@(*.yaml|*.json)');
  let index = -1;
...
function printOverallResults(results) {
  const noWarnings = results.successes.filter(s => s.report.warnings.length === 0).length;
  console.log('----------------------');
...
function getWarningsDistribution(results) {
  ...
  results.successes.forEach(suc => {
    ...
    const missingResponseSchema = suc.report.warnings.filter(w => w.type === 'MissingResponseSchema').length;
    if (typeof dist.MissingResponseSchema[missingResponseSchema] === 'undefined')
      dist.MissingResponseSchema[missingResponseSchema] = 0;
    dist.MissingResponseSchema[missingResponseSchema]++;

    const invalidSchemaType = suc.report.warnings.filter(w => w.type === 'InvalidSchemaType').length;
    ...
    const multipleResponses = suc.report.warnings.filter(w => w.type === 'MultipleResponses').length;
    ...
    const invalidSchemaTypeScalar = suc.report.warnings.filter(w => w.type === 'InvalidSchemaTypeScalar').length;
    if (typeof dist.InvalidSchemaTypeScalar[invalidSchemaTypeScalar] === 'undefined')
...
function printStats(results) {
  ...
  const numOpsMutation = results.successes.map(succ => succ.report.numOpsMutation);
  console.log(`Number of mutation operations:`);
  console.log(printSummary(numOpsMutation) + '\n');

  const numQueries = results.successes.map(succ => succ.report.numQueriesCreated);
  console.log(`Number of queries created:`);
  console.log(printSummary(numQueries) + '\n');

  const numMutations = results.successes.map(succ => succ.report.numMutationsCreated);
...
try {
  ...
  console.error(
    `Error: Please provide maximum number of APIs to check. ` + `For example:\n\n     npm run guru-test 10\n`
  );
  process.exit();
}
