...
function getFromObject(object: any, path: string, value?: any): any {
  ...
  const pathArray: string[] = Array.isArray(path) ? path : path.split(/[,[\].]/g).filter(Boolean);
  // Find value if exist return otherwise return undefined value;
  return pathArray.reduce((prevObj, key) => prevObj && prevObj[key], object) || value;
}

export async function getPackageProtoDefinition(protoFile: string, packageName: string): Promise<AnyNestedObject> {
