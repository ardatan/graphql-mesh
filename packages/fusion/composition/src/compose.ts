...
export function getAnnotatedSubgraphs(
  ...
  for (const subgraphConfig of subgraphs) {
    ...
    const fieldMapper: FieldMapper = (fieldConfig, fieldName) => {
      ...
      const typeNameSourceDirectives = getDirectiveExtensions(fieldConfig.type.ofType || fieldConfig.type)?.source;
      // Find the @source directive that has the type's name by ignoring the one that might be present for type translation.
      const typeNameSourceDirectiveName = typeNameSourceDirectives?.find(sourceDirective => !sourceDirective.type)?.name;
      ...
      const filteredSourceDirectives = sourceDirectives.filter(sourceDirective => {
          ...
          return sourceDirective.name !== fieldName ||
              ...
              (sourceDirective.type !== fieldConfig.type.toString() && sourceDirective.type === typeNameSourceDirectiveName);
      });
      ...
      for (const filteredSourceDirective of filteredSourceDirectives) {
          ...
          const isTypeMapped = filteredSourceDirective.type === fieldConfig.type?.toString() ||
            // The type's @source directive name will not have list or non-null representation.
            filteredSourceDirective.type.search(new RegExp(`^\\[?${typeNameSourceDirectiveName}!?\\]?!?$`)) > -1;

          if (!isTypeMapped) {
              filteredSourceDirective.type =
                fieldConfig.type?.toString().replace(/^(\[)?(.*)(!?\]!?|!)$/, `$1${typeNameSourceDirectiveName || '$2'}$3`);
          }
      }
