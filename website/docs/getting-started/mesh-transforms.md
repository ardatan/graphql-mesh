...
> NOTE: "wrap" is the only approach that works with data sources that already "speaks" GraphQL, or when you want to transform at all-sources (root) level, unless you're using [merger-bare](/docs/api/modules/merger-bare/). If you want to remove the possible runtime implications, consider either moving your `renames` at the data source level, or opting into `merger-bare`; in order to take advantage of "bare" mode.
