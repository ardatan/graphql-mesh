...
> **Disclaimer:** This section explains how to provide a custom server implementation that will be used by `mesh serve` and `mesh dev`. If you are looking for solutions to __deploy Mesh to Serverless environments__ or __not rely on `mesh serve`__, please refer to the ["How to: Deploy a Mesh Gateway"](/docs/getting-started/deploy-mesh-gateway) guide.
