...
All the documentation tutorials and guides rely on the "Books", "Authors" and "Stores" example APIs, available in [`examples/combine-multiple-source-types/`](https://github.com/Urigo/graphql-mesh/tree/master/examples/combine-multiple-source-types/):
...
Our goal is to build a Unified GraphQL Schema, through a Mesh Gateway that incorporate our 3 example services as follows:
...
In this tutorial, **we will first start by creating a Mesh configuration that incorporate the Books REST API**, as follows:
...
Our Books REST API provides an Open API definition file: [`examples/combine-multiple-source-types/books-service/openapi3-definition.json`](https://github.com/Urigo/graphql-mesh/tree/master/examples/combine-multiple-source-types/books-service/openapi3-definition.json).
...
Let's go to the [`examples/combine-multiple-source-types/mesh-configs/single-source/`](https://github.com/Urigo/graphql-mesh/tree/master/examples/combine-multiple-source-types/mesh-configs/single-source/) where the following dependencies are installed:
...
The `.meshrc.yml` configuration file contains the following content:
...
**Let's first start our Books API** by going to the [`examples/combine-multiple-source-types/books-service/`](https://github.com/Urigo/graphql-mesh/tree/master/examples/combine-multiple-source-types/books-service/) directory and run the following commands:

<PackageRun scripts={["install", "start"]} />

And, back to [`examples/combine-multiple-source-types/mesh-configs/single-source/`](https://github.com/Urigo/graphql-mesh/tree/master/examples/combine-multiple-source-types/mesh-configs/single-source/) directory, **let's start our GraphQL Gateway** by running:
...
Which gives us the following Books GraphQL Schema:
...
To remove a Query from the Unified Schema, we will install and configure the `filterSchema` _Transform_.
