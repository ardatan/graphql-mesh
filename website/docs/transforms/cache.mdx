...
> GraphQL Mesh uses a default localforage cache(it is fallbacked to LRU cache in NodeJS), but you can replace it with any other key=>value cache mechanism. See [cache section](/docs/cache/localforage) for more info.
