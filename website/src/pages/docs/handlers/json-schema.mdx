...
You can add query parameters to the request by two methods:

### Declaring the parameters

There are two methods to define the query parameters:
...
Query arguments could be defined globally, so they are added to all operations.
...
<Callout>Note that this example only declares the argument, but doesn't add it as query parameter</Callout>
