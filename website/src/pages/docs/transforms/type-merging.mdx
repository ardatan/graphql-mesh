---
description: "GraphQL Mesh enables the combination of multiple APIs with its type-merging transform. Merge types from various sources using Schema Stitching, preventing N+1 problems for requests. #GraphQL #Mesh"
---
...
using the [Type Merging](https://the-guild.dev/graphql/stitching/docs/approaches/type-merging) approach of Schema
Stitching.
...
<PackageCmd packages={['@graphql-mesh/transform-type-merging']} />
...
But it won't work because Mesh doesn't know which field belongs to where and how to combine those.
You could add `additionalResolvers` then extract `books` from `AuthorWithBooks` then return it as
`books` field of `Author` type, but this sounds a little overhead. So let's try Type Merging here.
