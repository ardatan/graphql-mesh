...
But it won't work because Mesh doesn't know which field belongs to which entity and how to combine them.
You could add `additionalResolvers` to extract `books` from `AuthorWithBooks` and then return it as
`books` field of `Author` type, but this adds a lot of overhead. So, let's try Type Merging here.
