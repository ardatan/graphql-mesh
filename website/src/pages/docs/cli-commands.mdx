...
Can have `--throwOnInvalidConfig=true` to make CLI throw in case of an invalid configuration. By default, CLI gives a warning and continues.
...
sources:
  - name: Cities
    handler:
      openapi:
        oasFilePath: https://api.apis.guru/v2/specs/mashape.com/geodb/1.0.0/swagger.json
    transforms:
      - rename:
          - from:
              type: Error
  ...
  - name: Weather
    handler:
      openapi:
        oasFilePath: https://api.apis.guru/v2/specs/weatherbit.io/2.0.0/swagger.json
    transforms:
      - rename:
          - from:
              type: Error
