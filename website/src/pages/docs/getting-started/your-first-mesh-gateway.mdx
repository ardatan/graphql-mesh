...
In particular, those Queries are prefixed with `AppController_` which is not very user-friendly.
...
Let's install the `rename` _Transform_ to rename the Query fields.
Loocking at the [`packages/single-source/`](https://github.com/charlypoly/graphql-mesh-docs-first-gateway/tree/master/packages/single-source)
...
Here, we are using a regexp to capture the end of the Query name, ignoring the `AppController_` prefix.
