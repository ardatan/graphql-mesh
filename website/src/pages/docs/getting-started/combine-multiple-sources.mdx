...
file (here, a Proto file) and the base URL or just the base URL if reflection is enabled.
Our "Authors" API supports reflection, so we can just provide the `endpoint` URL.
