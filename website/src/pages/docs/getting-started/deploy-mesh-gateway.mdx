...
import express from "@vendia/serverless-express/src/frameworks/express";
import { createBuiltMeshHTTPHandler } from "./.mesh";
...
export const handler: Handler = configure({
  ...
  app,
  // Specify the framework (default is KoaJS)
  framework: express,
});

```
If the lambda is behind an API Gateway, ensure the routing fits with the one specified in the `endpoint` setting from `.meshrc.yml`
