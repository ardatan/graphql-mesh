...
<Callout emoji="⚠️" type="warning">
  This method is only relevant to some of the handlers.
  Please make sure the handler you are using supports this method.
  You can do it by checking if handler's config reference (appearing on it's designated doc) includes the header fields mentioned in this doc.
</Callout>

## Setting configurations

There are two headers-designated fields under each handler in the configuration file - `schemaHeaders` and `operationHeaders`:
