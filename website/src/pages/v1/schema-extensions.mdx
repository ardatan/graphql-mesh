...
### Matching results by a field (`valueKeyField`)

Many GraphQL/REST/SQL collection endpoints (`WHERE id IN (...)`, Strapi filters, Hasura `where`, …)
return rows in their own order and may omit missing keys or return multiple rows per key. For those
sources, positional mapping is unsafe. Set `valueKeyField` to the field on each **result** used to
correlate it back to a batched key:
...
- `keyField` is still read from the **parent**
- `keysArg` still receives the batched key array
- `valueKeyField` is read from each **result** and used to build a key → result(s) map
- Mesh includes `valueKeyField` in the delegated selection automatically
- List fields get `[]` for missing keys (and all matches when several share a key); singular fields
  get `null` / the first match

If the query you’re delegating to don’t conform to these expectations and `valueKeyField` is not
enough, you can provide a custom resolver function in `additionalResolvers` to the Hive Gateway
