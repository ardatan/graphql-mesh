---
description:
  GraphQL Mesh Compose allows to compose non GraphQL and GraphQL sources into a unified graph
---
...
unified graph, so that you can serve it using GraphQL Mesh Serve.

If you compose your sources by using Federation, you can directly use
[GraphQL Mesh Serve](/v1/serve/introduction) to serve your unified graph.
...
### Extending the unified graph using `additionalTypeDefs`

You can extend your unified graph, by adding additional type definitions to the schema. Using this
...
### Transforming the unified graph using `transforms`

If you want to transform the unified graph instead of subgraphs, you can use the `transforms` option
on higher level, instead of subgraph level.
...
You can generate the unified graph by running the following command:

```sh
mesh-compose > unifiedgraph.graphql
```

<Callout>
  [In order to serve the unified graph, you need to setup GraphQL Mesh Serve](/v1/serve).
</Callout>
