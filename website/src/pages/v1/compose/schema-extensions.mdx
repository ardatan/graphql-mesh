...
available through [type merging](/v1/compose/type-merging). While type merging frequently eliminates
the need for schema extensions, it does not preclude their use.
...
Let's say we have a `subgraph A`, and the unified graph like below;
...
And together with other composed subgraphs or extensions, we have the following unified graph;
...
Suppose we want the unified graph to delegate retrieval of `repositories` to the `subgraph A`, in
order to execute queries such as this one:
...
To formalize this navigation within our unified graph, we can extend each type with a new field that
...
resolver function in `additionalResolvers` in `serve-runtime` configuration.
...
[Filter Transform](/v1/compose/transforms/filter) to hide it from the final schema. This is helpful
