---
description:
  Extend your unified graph by combining multiple GraphQL schemas and resolvers with GraphQL Mesh
  schema extensions.
---
...
need for [schema extensions](/v1/compose/schema-extensions) (though does not preclude their use). To
migrate from schema extensions, simply enable type merging and then start replacing extensions one
by one with merges.

This concept today is the most preferred one to combine multiple subgraphs in GraphQL such as Apollo
Federation.
...
`TODO: Add recipes` See related [handbook example](/v1/compose/recipes/type-merging-single-records)
for a working demonstration of this setup.
...
configuration. If your schema doesn't match this pattern, you can use
[transforms](/v1/compose/transforms) to manipulate the schema to match this pattern. Let's say you
don't have `userById` in `Posts` but `postUserById` instead. You can use the `rename` transform to
rename the field to match the pattern.

## Stitching Directives

This can also be written schema-type definitions using the `@merge` directive of the
[stitching SDL](https://the-guild.dev/graphql/stitching/docs/approaches/stitching-directives).
...
`TODO: Extend this documentation by using https://the-guild.dev/graphql/stitching/docs/approaches/type-merging`
