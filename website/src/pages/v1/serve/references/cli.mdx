...
Options:
  --fork <count>                          count of workers to spawn. defaults to `os.availableParallelism()` when NODE_ENV is "production", otherwise only one (the
                                          main) worker (default: 1, env: FORK)
  ...
  --native-import                         use the native "import" function for importing the config file
  --hive-registry-token <token>           Hive registry token for usage metrics reporting (env: HIVE_REGISTRY_TOKEN)
  --apollo-graph-ref <graphRef>           graph ref of the managed federation graph. (<YOUR_GRAPH_ID>@<VARIANT>) (env: APOLLO_GRAPH_REF)
  --apollo-key <apiKey>                   API key to use to authenticate with the managed federation up link (env: APOLLO_KEY)
...
Commands:
  ...
  proxy [options] [endpoint]              serve a proxy to a GraphQL API and add additional features such as monitoring/tracing, caching, rate limiting, security, and
                                          more
...
- `HIVE_CDN_KEY`: The API key provided by GraphQL Hive to fetch the supergraph.
