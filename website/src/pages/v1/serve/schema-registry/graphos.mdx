...
By default, Mesh will use standard Apollo GraphOS environment variables `APOLLO_GRAPH_REF` and
`APOLLO_KEY`.
...
```sh
npx mesh-serve supergraph --apollo-graph-ref <graph_id>[@<variant>] --apollo-key <api_key>
```
...
If you want to use GraphOS to report usage metrics, you can still provide the CLI arguments as in
above. Or you can use a configuration file, which allows further customization for the
configuration:
