...
You're recommended to use the `gateway.config.ts` file to configure Mesh Serve. Simply mount the
...
docker run \
  ...
  ghcr.io/ardatan/mesh-serve proxy
```
...
docker run \
  ...
  ghcr.io/ardatan/mesh-serve supergraph --port=8080
```
...
```sh
docker build -t mesh-serve-w-block-suggestions .
```
...
docker run \
  -p 4000:4000 \
  -v "$(pwd)/gateway.config.ts:/serve/gateway.config.ts" \
  mesh-serve-w-block-suggestions supergraph
```
...
```sh
docker run -p 4000:4000 mesh-serve-w-my-timing supergraph
```
...
docker run -p 4000:4000 \
  ...
  mesh-serve-w-my-timing supergraph
```
...
```sh
docker run -p 4000:4000 mesh-serve-w-add-res supergraph
```
