...
Once you configured, and tested your gateway. Now, it is time to deploy it. GraphQL Mesh Serve
Runtime uses Web Standards (WHATWG Fetch API) not only as an HTTP client but also for handling the
...
this case, we will show here how to pass the request information from your environment to Mesh, then
get the response for your environment back.
...
const serveRuntime = createServeRuntime<ImaginaryEnvironmentServerContext>({
  supergraph: () => getMySupergraph()
})

// Let's say it needs a function exported
export async function meshEndpoint(
  ...
  const res = await serveRuntime.fetch(envRequest.url, {
