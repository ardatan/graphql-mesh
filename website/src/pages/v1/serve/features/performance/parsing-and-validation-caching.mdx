...
By default, GraphQL Mesh maintains a parsing and validation cache. If requests contain documents
...
```ts filename="mesh.config.ts" {4,5}
import { defineConfig } from '@graphql-mesh/serve-cli'

export const gatewayConfig = defineServe({
...
```ts filename="mesh.config.ts" {9-13}
import { defineConfig } from '@graphql-mesh/serve-cli'
