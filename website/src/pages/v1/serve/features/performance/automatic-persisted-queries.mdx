...
<Callout>
  ...
  Operations](/v1/serve/features/performance/persisted-operations).
...
```ts filename="Automatic Persisted Queries Mesh setup" {3, 13}
...
Start your Mesh server and send a request for priming the cache (register the operation).
