...
GraphQL Mesh is capable of handling compressions in the following directions. We can selectively
...
In your Mesh configuration, you need to enable the compression plugin for the gateway.
...
Now Mesh will respect the `Accept-Encoding` header from the client and compress the response
accordingly.
