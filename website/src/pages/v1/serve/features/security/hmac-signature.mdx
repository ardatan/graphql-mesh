...
To get started with HMAC signing in your GraphQL Mesh Gateway, you need to add the
`@graphql-mesh/hmac-upstream-signature` package to your project:
...
<Callout>
  Make sure to add `useHMACUpstreamSignature` last in the plugins list in your Mesh configuration.
</Callout>
...
If you are using Yoga, you can install the same plugin package:
...
The `useHMACUpstreamSignature` plugin allow you to customize the HMAC signing behavior in the Mesh
gateway:
