...
This Mesh plugin implements HMAC signing for requests between GraphQL Mesh gateway and the upstream
...
signed by the Mesh gateway. In case of any missing signature, tampering or unauthorized access, the
...
The secret key should be a random, opaque string, that will be shared between the Mesh Gateway and
the subgraphs validating the HMAC signature.

### Step 2: HMAC Signing in Mesh Gateway
...
import { useHmacSignatureValidation } from '@graphql-mesh/serve-runtime'
...
verify the HMAC signature. You can still use the utilities from the `@graphql-mesh/serve-runtime`
library to serialize the request parameters and verify the HMAC signature in a stable way.

Start by installing the `@graphql-mesh/serve-runtime` package:
...
import { defaultParamsSerializer } from '@graphql-mesh/serve-runtime'
...
- The HMAC signature should be calculated using the secret key shared between the Mesh Gateway and
...
> The signature is produced by the Mesh Gateway using the shared secret key, and the serialized
...
The `hmacUpstreamSignature` flag allows you to customize the HMAC signing behavior in the Mesh
gateway:

- `secret`: The secret key used for HMAC signing and verification. It should be a random, opaque
  string shared between the Mesh Gateway and the subgraph services.
