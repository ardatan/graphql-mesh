...
We recommend using HMAC signing for requests between the GraphQL Mesh gateway and the upstream in
cases where authentication plugins are involved, in order to ensure the gateway is the only entity
that can execute requests to the subgraph on behalf of the end-users.

You can use the [HMAC Signature plugin](/v1/serve/features/security/hmac-signature) to perform
requesting signing and verification.
