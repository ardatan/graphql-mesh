...
To pass the full token payload, you can use the `forwared.claims` option:
...
[/v1/serve/features/auth/generic-auth](Generic Auth plugin) to have a granular protection using with
or without JWT.
...
```ts
import { useGenericAuth, useJWT } from '@graphql-mesh/plugin-jwt-auth'
