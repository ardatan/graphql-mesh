...
use [HMAC Signature](/v1/serve/features/security/hmac-signature) between your Mesh Gateway and the
...
```ts filename="gateway.config.ts"
import { defineConfig, createInlineSigningKeyProvider, createRemoteJwksSigningKeyProvider, extractFromHeader, extractFromCookie } from '@graphql-mesh/serve-cli'
...
The JWT plugin will inject the decoded token and payload into the context of the GraphQL Mesh
gateway.
...
import { useForwardedJWT } from '@graphql-mesh/serve-runtime'
...
```ts
import { defineConfig, extractFromHeader } from '@graphql-mesh/serve-cli'
...
```ts
import { extractFromCookie, defineConfig } from '@graphql-mesh/serve-cli'
...
```ts
import { defineConfig, extractFromCookie, extractFromHeader } from '@graphql-mesh/serve-cli'
...
```ts
import { createInlineSigningKeyProvider, defineConfig } from '@graphql-mesh/serve-cli'
...
```ts
import { createRemoteJwksSigningKeyProvider, defineConfig } from '@graphql-mesh/serve-cli'
