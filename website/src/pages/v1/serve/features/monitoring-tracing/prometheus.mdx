...
description: Learn how to track and analyze GraphQL execution metrics using
@graphql-mesh/plugin-prometheus. Expose and collect data to optimize your gateway.
...
set `fetchRequestHeaders` option to `true` to enable it.

##### `responseHeaders`

A JSON encoded object containing the headers of the upstream response. This is disabled by default,
set `fetchResponseHeaders` option to `true` to enable it.
