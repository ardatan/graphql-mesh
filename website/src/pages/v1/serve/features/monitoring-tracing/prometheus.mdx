...
In the metrics, you will find the timming of each phase of the GraphQL execution. If you are not
familiar with the lifecycle of a GraphQL operation in Mesh, please refer to the
[Plugin Lifecycle page](/v1/serve/features/custom-plugins#plugin-lifecycle). Each plugin hook have a
corresponding metric which tracks timmings as
...
process each incoming request as an
...
Please not that this metric is not specific to GraphQL, it tracks all incoming HTTP requests.
...
| Label           | Enabled by default | Descirption                                                                                                                                                                                                                                                                                                                                                                             |
| --------------- | :----------------: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `method`        |  Not configurable  | The HTTP method used to request the gateway endpoint.<br/><br/> Since GraphQL usually only uses `POST` requests, this can be used to filter out GraphiQL related requests. <br/><br/> It can be any HTTP verb, including unallowed ones. Which means this metric can also be used to track malformed or malicious requests.                                                             |
...
made using the `fetch` function provided by Mesh. It is reported as an
[histogram](https://prometheus.io/docs/concepts/metric_types/#histogram).

This metric can provide insights into the network usage of your gateway. It does not only include
requests made to resolve GraphQL operation responses, but also any other outgoing HTTP requests made
by Mesh or one of its plugins. It will for example include requests made to fetch the supergraph
schema from the configured Schema Registry.
...
queries made to resolve incoming operations as an
[histogram](https://prometheus.io/docs/concepts/metric_types/#histogram).
...
This metric tracks the number of errors that occurred during subgraph execution. It counts all
...
spent parsing the incoming graphql operation. It is reported as an
[histogram](https://prometheus.io/docs/concepts/metric_types/#histogram).
...
time spent validating the incoming graphql operation. It is reported as an
[histogram](https://prometheus.io/docs/concepts/metric_types/#histogram).
...
spent building the context object that will be passed to the executors. It is reported as an
[histogram](https://prometheus.io/docs/concepts/metric_types/#histogram).
...
the data from all sources required to construct the final response. It is reported as an
[histogram](https://prometheus.io/docs/concepts/metric_types/#histogram).
...
is reported as an [histogram](https://prometheus.io/docs/concepts/metric_types/#histogram).

It will notably include the time spent to setup upstream subscriptions with apropiate transport for
each sources.
...
spent in the GraphQL specific processing, excluding the HTTP level processing. It is reported as an
[histogram](https://prometheus.io/docs/concepts/metric_types/#histogram).
...
same timming than [`graphql_envelop_request_duration`](#graphql_envelop_request_duration) but as a
...
responses, but it also include errors from other GraphQL processing phases (parsing, validation and
...
Depending on the phase where the error occurred, some labels may be missing. For example, if the
...
This metric tracks the number of GraphQL operation executed. It counts all operations, either failed
or successful, including subscriptions. It is exposed as a
[counter](https://prometheus.io/docs/concepts/metric_types/#counter).
...
To mitigate the cost of instumenting all resolvers, you can explicitly list the fields that should
...
This metric tracks the number of schema changes that occurred since the gateway started. When
polling is enabled, this will include the schema reloads.

If you are using a plugin that modify the schema on the fly, be aware that this metric will also
...
additional resolvers, not on field that are resolved by a subgraph. It is up to the subgraph server
