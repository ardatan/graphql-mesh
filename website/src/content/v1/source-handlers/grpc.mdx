...
When using gRPC reflection (no `source`), use `reflectionMetadata` to send metadata only on reflection
requests — for example routing headers required by your infrastructure:
